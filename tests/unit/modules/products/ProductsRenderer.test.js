...
describe('ProductsRenderer', () => {
  ...
  describe('constructor', () => {
    it('should initialize with correct elements', () => {
      ...
      expect(renderer.countElement).toBe(document.getElementById('product-count'));
      expect(renderer.loadMoreContainer).toBe(document.getElementById('load-more-container'));
      expect(renderer.searchInput).toBe(document.getElementById('search-input'));
      expect(renderer.sortSelect).toBe(document.getElementById('sort-select'));
      expect(renderer.priceSlider).toBe(document.getElementById('price-slider'));
      expect(renderer.priceValue).toBe(document.getElementById('price-value'));
      expect(renderer.resetButton).toBe(document.getElementById('reset-filter'));
    });
  ...
  describe('render', () => {
    it('should render products and update count', () => {
      ...
      expect(renderer.loadMoreContainer.classList.contains('hidden')).toBe(true);
    });
    ...
    it('should handle missing container gracefully', () => {
      ...
      expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining('Product grid not found'));
  ...
  describe('append', () => {
    ...
    it('should do nothing when products empty', () => {
      renderer.render(mockProducts, 2);
      const initialCount = document.getElementById('product-grid').children.length;
      renderer.append([]);
      expect(document.getElementById('product-grid').children.length).toBe(initialCount);
    });
  });
  ...
  describe('updateLoadMore', () => {
    it('should show load more when hasMore true', () => {
      ...
      expect(renderer.loadMoreContainer.classList.contains('hidden')).toBe(false);
    });
  ...
  describe('getUIState', () => {
    it('should return current UI state', () => {
      ...
      select.innerHTML = '<option value="default">Default</option><option value="price-asc">Price Asc</option>';
